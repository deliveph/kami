;
(function ($) {
    $.fn.rightcircleMagic = function (options) {
        let width, height, largeContainer, canvas, ctx, target, animateHeader = true;
        let circles = [];
        let settings = $.extend({
            elem: '.header',
            color: 'rgba(255,255,255,.4)',
            radius: 20,
            densety: 0.3,
            clearOffset: 0.2,
            direction: 'top',
            left: "40%",
            top: "105px"
        }, options);
        initContainer();
        //addListeners();

        function initContainer() {
            if (settings.direction != "top") {
                width = 300;
                height = $(".about_content").outerHeight(true);
            } else {
                width = $(settings.elem).first("img").width() / 2.5; // window.innerWidth;
                height = $(settings.elem).first("img").height(); // window.innerHeight;
            }

            target = {
                x: 0,
                y: height
            };
            largeContainer = document.querySelector(settings.elem);
            largeContainer.style.height = height + 'px';
            initCanvas();
            canvas = document.getElementById('rightcanvas');
            canvas.width = width;
            canvas.height = height;
            ctx = canvas.getContext('2d');
            for (let x = 0; x < width * settings.densety; x++) {
                let c = new Circle();
                circles.push(c);
            }


            animate();
        }

        function initCanvas() {
            let canvasElement = document.createElement('canvas');
            canvasElement.id = 'rightcanvas';
            canvasElement.style.position = "absolute";
            canvasElement.style.left = settings.left;
            canvasElement.style.top = settings.top;
            canvasElement.style.zIndex = 0;
            largeContainer.append(canvasElement);
        }

        function addListeners() {
            //window.addEventListener('scroll', scrollCheck);
            //window.addEventListener('resize', resize);
        }

        function scrollCheck() {
            if (document.body.scrollTop > height) animateHeader = false;
            else animateHeader = true;
        }

        function resize() {
            width = $(settings.elem).first("img").width() / 2.5; // window.innerWidth;
            height = $(settings.elem).first("img").height() - 5; // window.innerHeight;
            largeContainer.style.height = height - 100 + 'px';
            canvas.width = width;
            canvas.height = height;
        }

        function animate() {
            if (animateHeader) {
                ctx.clearRect(0, 0, width, height);
                for (let i in circles) {
                    circles[i].draw();
                }
            }
            requestAnimationFrame(animate);
        }

        function randomColor() {
            let r = Math.floor(Math.random() * 255);
            let g = Math.floor(Math.random() * 255);
            let b = Math.floor(Math.random() * 255);
            let alpha = Math.random().toPrecision(2);
            let rgba = `rgba(${r}, ${g}, ${b}, ${alpha})`;
            return rgba;
        };

        function Circle() {
            var self = this;
            (function () {
                self.pos = {};
                init();
            })();

            function init() {

                if (settings.direction == 'top') {
                    self.pos.x = Math.random() * width;
                    self.pos.y = height + Math.random() * 100;
                } else if (settings.direction == 'right') {
                    self.pos.x = 0;
                    self.pos.y = height / 2 + height * Math.random();
                } else if (settings.direction == 'left') {
                    self.pos.x = width;
                    self.pos.y = height * Math.random();
                }

                self.alpha = 0.1 + Math.random() * settings.clearOffset;
                self.scale = 0.1 + Math.random() * 0.3;
                self.speed = Math.random();
                if (settings.color == 'random') {
                    self.color = randomColor();
                } else {
                    self.color = settings.color;
                }
            }
            this.draw = function () {
                if (self.alpha <= 0) {
                    init();
                }

                if (settings.direction == 'top') {
                    self.pos.y -= self.speed;
                } else if (settings.direction == 'right') {
                    self.pos.x += self.speed;
                } else if (settings.direction == 'left') {
                    self.pos.x -= self.speed;
                }

                self.alpha -= 0.0005;
                ctx.beginPath();
                ctx.arc(self.pos.x, self.pos.y, self.scale * settings.radius, 0, 2 * Math.PI, false);
                //ctx.fillStyle = self.color;
                //ctx.fill();
                ctx.strokeStyle = self.color;
                ctx.stroke();
                ctx.closePath();
            };
        }
    }
})(jQuery);